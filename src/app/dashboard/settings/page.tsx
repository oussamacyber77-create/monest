    ...
    <div className="flex-1 p-4 md:p-6 overflow-y-auto space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {sections.map((s) => {
          ...
          if (s.external) {
            return (
              <a key={s.label.en} href={s.href} target="_blank" rel="noopener noreferrer" className="p-6 border border-[#D4D4D4] dark:border-[#333333] bg-white dark:bg-[#0D0D0D] hover:bg-[#E8E8E8] dark:hover:bg-[#1A1A1A] transition-colors flex gap-4 items-center">
                {content}
              </a>
            )
          }
          ...
          return (
            <button key={s.label.en} onClick={() => s.href && router.push(s.href)} className="p-6 border border-[#D4D4D4] dark:border-[#333333] bg-white dark:bg-[#0D0D0D] hover:bg-[#E8E8E8] dark:hover:bg-[#1A1A1A] transition-colors flex gap-4 items-center text-start">
              {content}
            </button>
          )
        })}
