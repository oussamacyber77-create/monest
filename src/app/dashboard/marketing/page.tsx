...
import { bestCampaigns, worstCampaigns } from "@/lib/mock-data/dashboard"
import { TrendingUp, TrendingDown, Megaphone, Plus, BarChart3, Target, Lightbulb, Play, Pause, DollarSign, Eye, MousePointerClick, CalendarDays, ArrowUpRight, Download, X, Check, LayoutGrid, FileText, PenLine, PieChart } from "lucide-react"
import { SARIcon } from "@/components/ui/sar-icon"

type Tab = "overview" | "campaigns" | "reports"
...
const platforms = [
  {
    ...
    color: "#4285F4",
    cpa: 45, roas: 3.2, cvr: 2.8, spend: 28500, revenue: 91200,
  },
  {
    name: "Meta Ads",
    icon: (
    ...
    color: "#1877F2",
    cpa: 38, roas: 4.1, cvr: 3.5, spend: 34200, revenue: 140220,
  },
  {
    name: "TikTok Ads",
    icon: (
    ...
    color: "#000000",
    darkColor: "#F2F2F2",
    cpa: 29, roas: 2.6, cvr: 1.9, spend: 18000, revenue: 46800,
  },
]
...
const sidebarItems = [
  { key: "overview", label: { ar: "نظرة عامة", en: "Overview" }, icon: LayoutGrid, color: "#3B82F6" },
  { key: "campaigns", label: { ar: "الحملات", en: "Campaigns" }, icon: Megaphone, color: "#8B5CF6" },
  { key: "reports", label: { ar: "التقارير", en: "Reports" }, icon: FileText, color: "#10B981" },
  { key: "create", label: { ar: "إنشاء حملة", en: "Create Campaign" }, icon: PenLine, color: "#F59E0B" },
]
...
export default function MarketingPage() {
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      {/* Sidebar */}
      <div className="hidden md:flex flex-col w-56 shrink-0 border-e border-[#D4D4D4] dark:border-[#333333] bg-white dark:bg-[#0D0D0D]">
        <div className="p-4 border-b border-[#D4D4D4] dark:border-[#333333]">
          <h2 className="text-sm font-bold text-[#0D0D0D] dark:text-[#F2F2F2]">{lang === "ar" ? "مركز التسويق" : "Marketing"}</h2>
          <p className="text-[10px] text-[#999999] mt-0.5">{lang === "ar" ? "تحليل وإدارة الحملات" : "Campaign management"}</p>
        </div>
        <div className="flex-1 py-2">
          {sidebarItems.map(item => (
            <button key={item.key} onClick={() => {
              if (item.key === "create") setShowNewCampaign(true)
              else setActiveTab(item.key as Tab)
            }}
              className={"w-full flex items-center gap-2.5 px-4 py-2.5 text-xs font-medium transition-colors " + (activeTab === item.key ? "bg-[#F2F2F2] dark:bg-[#1A1A1A]" : "text-[#666666] dark:text-[#999999] hover:text-[#0D0D0D] dark:hover:text-[#F2F2F2] hover:bg-[#F9F9F9] dark:hover:bg-[#111111]")}>
              <item.icon size={16} style={activeTab === item.key ? { color: item.color } : {}} />
              {item.label[lang]}
            </button>
          ))}
        </div>
        <div className="p-4 border-t border-[#D4D4D4] dark:border-[#333333]">
          <div className="flex items-center gap-2 p-2 bg-[#F9F9F9] dark:bg-[#111111] rounded">
            <PieChart size={14} className="text-[#666666]" />
            <div className="text-[10px]">
              <p className="font-bold text-[#0D0D0D] dark:text-[#F2F2F2]">{lang === "ar" ? "ROAS الإجمالي" : "Total ROAS"}</p>
              <p className="text-green-600 font-bold">3.6x</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-5">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <button onClick={() => setActiveTab("reports")} className="h-9 px-3 border border-[#D4D4D4] dark:border-[#333333] text-[11px] font-medium text-[#666666] hover:text-[#0D0D0D] dark:text-[#999999] dark:hover:text-[#F2F2F2] transition-colors flex items-center gap-1.5">
              <FileText size={13} />
              {lang === "ar" ? "تقارير" : "Reports"}
            </button>
            <button onClick={() => setShowNewCampaign(true)} className="h-9 px-3 text-[11px] font-bold text-white flex items-center gap-1.5 hover:opacity-90 transition-opacity"
        ...
        {/* Mobile tabs */}
        <div className="md:hidden flex gap-1 bg-[#E8E8E8] dark:bg-[#1A1A1A] p-1">
          {(["overview", "campaigns", "reports"] as const).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={"flex-1 py-2 text-[11px] font-medium transition-colors " + (activeTab === tab ? "bg-white dark:bg-[#0D0D0D] text-[#0D0D0D] dark:text-[#F2F2F2]" : "text-[#666666] hover:text-[#0D0D0D] dark:text-[#999999] dark:hover:text-[#F2F2F2]")}>
        ...
        {activeTab === "overview" && (
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { icon: Megaphone, label: { ar: "الحملات النشطة", en: "Active Campaigns" }, value: "3", change: "+2", color: "#3B82F6" },
                { icon: DollarSign, label: { ar: "إجمالي الإنفاق", en: "Total Spend" }, value: "80,700", change: "+12%", color: "#F59E0B" },
                { icon: Target, label: { ar: "معدل التحويل", en: "Conversion Rate" }, value: "3.8%", change: "+0.6%", color: "#10B981" },
                { icon: TrendingUp, label: { ar: "متوسط ROAS", en: "Avg ROAS" }, value: "3.6x", change: "+15%", color: "#8B5CF6" },
              ].map((s) => (
                <div key={s.label.en} className="border border-[#D4D4D4] dark:border-[#333333] bg-white dark:bg-[#0D0D0D] p-4">
                  ...
                  <p className="text-[10px] text-[#999999] mb-0.5">{s.label[lang]}</p>
              ...
              {platforms.map((p) => (
                <div key={p.name} className="border border-[#D4D4D4] dark:border-[#333333] bg-white dark:bg-[#0D0D0D] p-4">
                  ...
                  <div className="grid grid-cols-2 gap-2.5 text-xs">
                    <div>
                      <span className="text-[10px] text-[#999999]">CPA</span>
                      <p className="font-bold text-[#0D0D0D] dark:text-[#F2F2F2] mt-0.5">{p.cpa} <SARIcon className="w-2.5 h-2.5 inline" /></p>
                    </div>
                    <div>
                      <span className="text-[10px] text-[#999999]">ROAS</span>
                      <p className="font-bold text-green-600 dark:text-green-400 mt-0.5">{p.roas}x</p>
                    </div>
                    <div>
                      <span className="text-[10px] text-[#999999]">CVR</span>
                      <p className="font-bold text-[#0D0D0D] dark:text-[#F2F2F2] mt-0.5">{p.cvr}%</p>
                    </div>
                    <div>
                      <span className="text-[10px] text-[#999999]">{lang === "ar" ? "إنفاق" : "Spend"}</span>
                      <p className="font-bold text-[#0D0D0D] dark:text-[#F2F2F2] mt-0.5">{p.spend.toLocaleString("en-US")} <SARIcon className="w-2.5 h-2.5 inline" /></p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="border border-[#D4D4D4] dark:border-[#333333] bg-white dark:bg-[#0D0D0D]">
            ...
            <div className="divide-y divide-[#D4D4D4] dark:divide-[#333333]">
              {allCampaigns.map((camp, i) => {
                const pf = platforms.find(p => p.name === camp.platform)
                return (
                      ...
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1 mt-0.5">
                            {pf && <div className="w-3.5 h-3.5 flex items-center justify-center">{pf.icon}</div>}
                            <span className="text-[10px] text-[#999999]">{camp.platform}</span>
                          </div>
                        </div>
                      </div>
        ...
        {activeTab === "reports" && (
          <div className="space-y-4">
            <div className="grid md:grid-cols-3 gap-3">
              {[
                { label: { ar: "إجمالي الإيرادات", en: "Total Revenue" }, value: "278,220", change: "+18%", color: "text-green-600" },
                { label: { ar: "إجمالي الإنفاق", en: "Total Spend" }, value: "80,700", change: "+12%", color: "text-amber-600" },
                { label: { ar: "صافي الربح", en: "Net Profit" }, value: "197,520", change: "+22%", color: "text-green-600" },
              ].map((r) => (
                <div key={r.label.en} className="border border-[#D4D4D4] dark:border-[#333333] bg-white dark:bg-[#0D0D0D] p-4">
                  ...
                  <p className="text-xl font-bold text-[#0D0D0D] dark:text-[#F2F2F2]">{r.value} <SARIcon className="w-3.5 h-3.5 inline" /></p>
                ...
                <tbody className="divide-y divide-[#D4D4D4] dark:divide-[#333333]">
                  {[
                    ...
                    <tr key={i} className="hover:bg-[#F2F2F2] dark:hover:bg-[#1A1A1A]">
                      ...
                      <td className="py-2.5 text-[#666666]">{r.spend.toLocaleString("en-US")}</td>
                      <td className="py-2.5 text-green-600 font-medium">{r.revenue.toLocaleString("en-US")}</td>
            ...
            <div className="relative w-full max-w-lg bg-[#F2F2F2] dark:bg-[#0D0D0D] border border-[#D4D4D4] dark:border-[#333333] p-5 max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {platforms.map(p => (
                      <button key={p.name} className="flex-1 h-10 border border-[#D4D4D4] dark:border-[#333333] flex items-center justify-center gap-1.5 text-xs hover:bg-[#E8E8E8] dark:hover:bg-[#333333] transition-colors">
                        <div className="w-4 h-4 flex items-center justify-center">{p.icon}</div>
                        {p.name}
                      </button>
                    ))}
                  </div>
                ...
                <button className="w-full h-11 text-white text-sm font-bold hover:opacity-90 transition-opacity" style={{ backgroundColor: primaryColor }}>
